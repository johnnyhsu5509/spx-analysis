...
def _breadth(index_day=None):
    ...
    rule51（2026-09-03 新增）：舊版只取序列末兩筆算變動，**不檢查末日是否與指數對齊**，
    會把前一日的廣度數字靜默當成當日輸出。

    **真正的機制（由 NDX 系統同日獨立查明，機制描述以其為準）**：
    不是「ETF 日線落後」，而是 **Yahoo 日線最新一列的 Close 為 NaN**——
    OHL 與 Volume 都在，只缺 Close。`dropna(subset=["Close"])` 之後那一列整個消失，
    程式取到的「最後一筆」其實是前一日。**不報錯，只靜默回傳昨天的數字。**
    與 gap_fix 處理的「中間缺日」機制不同：中間缺日會被補，**尾端 NaN 不會**。

    2026-09-02 實例：SPX 側 RSP/SPY、NDX 側 QQQ/QQQE/SOXX 同時發生，
    兩套系統都是「察覺 breadth 三個數字與前一日逐字相同」才發現。
