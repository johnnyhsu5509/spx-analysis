...
PATCH_DAYS = {"^NDX": [], "^GSPC": [], "^VXN": [],
              "QQQE": ["2026-09-22"], "QQQ": ["2026-09-22"], "SOXX": ["2026-09-22"]}
...
with open("ndx_aux_0923.txt","w",encoding="utf-8") as f:
...
bars = [{"t": str(i)[5:16], "o": round(float(r.Open),1), "h": round(float(r.High),1), "l": round(float(r.Low),1), "c": round(float(r.Close),1)} for i, r in zip(idx, d.itertuples()) if str(i.date()) == "2026-09-22"]
q = qqq.tail(3)
out2 = json.load(open("ndx_aux_0923.txt", encoding="utf-8"))
out2["ndx_30m_0922"] = bars
...
json.dump(out2, open("ndx_aux_0923.txt","w",encoding="utf-8"), indent=1, ensure_ascii=False)
