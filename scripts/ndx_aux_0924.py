...
PATCH_DAYS = {"^NDX": ["2026-09-22"], "^GSPC": [], "^VXN": ["2026-09-22"],
              "QQQE": ["2026-09-22", "2026-09-23"], "QQQ": ["2026-09-23"], "SOXX": ["2026-09-23"]}
...
out, plog = {}, {}
data = {}
for t in ["^NDX", "^GSPC", "QQQ", "QQQE", "SOXX", "^VXN", "^VIX", "^TNX", "DX-Y.NYB", "BZ=F", "CL=F"]:
    if t in PATCH_DAYS:
        data[t], plog[t] = get(t)
    else:
...
for t, k in [("^VXN","vxn"), ("^VIX","vix"), ("^TNX","tnx"), ("DX-Y.NYB","dxy"), ("BZ=F","brent"), ("CL=F","wti")]:
    s = data[t]["Close"]
...
with open("ndx_aux_0924.txt","w",encoding="utf-8") as f:
    json.dump(out,f,indent=2,ensure_ascii=False)
print("OK")
d = yf.download("^NDX", period="5d", interval="30m", progress=False, auto_adjust=False)
if isinstance(d.columns, pd.MultiIndex):
    d.columns = d.columns.get_level_values(0)
idx = pd.to_datetime(d.index)
try: idx = idx.tz_convert("America/New_York")
except Exception: pass
bars = [{"t": str(i)[5:16], "o": round(float(r.Open),1), "h": round(float(r.High),1), "l": round(float(r.Low),1), "c": round(float(r.Close),1)} for i, r in zip(idx, d.itertuples()) if str(i.date()) == "2026-09-23"]
q = qqq.tail(3)
out2 = json.load(open("ndx_aux_0924.txt", encoding="utf-8"))
out2["ndx_30m_0923"] = bars
out2["qqq_last3"] = [{"d": str(i.date()), "c": round(float(r.Close),2), "v": None if pd.isna(r.Volume) else int(r.Volume)} for i, r in zip(q.index, q.itertuples())]
ath = ndx["Close"].idxmax(); out2["ndx_max_close"] = [str(ath.date()), round(float(ndx["Close"].max()),2)]
out2["ndx_max_high"] = round(float(ndx["High"].max()),2)
json.dump(out2, open("ndx_aux_0924.txt","w",encoding="utf-8"), indent=1, ensure_ascii=False)
